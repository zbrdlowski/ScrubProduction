...
if (!function_exists('customOrdersActivityActionLabel')) {
  function customOrdersActivityActionLabel(string $action): string
  {
    ...
    return $map[$action] ?? ucwords(str_replace('_', ' ', $action ?: 'activity'));
  }
}

if (!function_exists('customOrdersActivityDetail')) {
  function customOrdersActivityDetail(array $activity): string
  {
    ...
    $payloadRaw = trim((string) ($activity['payload'] ?? ''));
    if ($payloadRaw !== '') {
      $payload = json_decode($payloadRaw, true);
      if (is_array($payload)) {
        foreach (['title', 'message', 'detail', 'note'] as $key) {
