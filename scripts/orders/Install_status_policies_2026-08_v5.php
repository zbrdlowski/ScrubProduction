...
/**
 * install_status_policies_2026_08_v5.php
 * ---------------------------------------------------------
 * 1) Doplna status_definitions: item P/CHECK_STOCK, item
 *    F/PLASTICS_IN_STOCK, order-level PLASTICS_IN_STOCK,
 *    order-level INFO_REQUIRED (novy status - 'Info required').
 * 2) UPSERT VSETKY status_workflow_rules:
 *    - Plastics Check Stock (tier 0)
 *    - Info Required / Info Requested / Communication (tier 1,
 *      teraz rozdelene: _REQUIRED kody -> Info Required,
 *      _REQUESTED kody -> Info Requested)
 *    - Ready - VSETKYCH 15 neprazdnych podmnozin {G,S,P,F}
 *      (predtym len 9 z DEPT_PREFIX_MAP, chybali kombinacie
 *      ktore vznikaju z viacerych samostatnych poloziek v
 *      jednej objednavke, napr. G+P)
 *    - Delay, In Progress (bez zmeny)
 *    - New - tiez VSETKYCH 15 kombinacii (rovnaky dovod ako Ready)
 *
 * POUZITIE: nahraj do scripts/orders/ a spusti RAZ cez browser
 * (ako admin) alebo cez CLI: php install_status_policies_2026_08_v5.php
 * Bezpecne spustit opakovane - vsetko je upsert/skip-if-exists.
 * Tento skript NAHRADZA v3 aj v4 - staci spustit len tento.
 */
...
foreach ($newStatusDefinitions as $def) {
    ...
    $insStmt = $conn->prepare("
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ");
    $insStmt->bind_param('sssssii', $def['scope'], $def['department'], $def['code'], $def['label'], $def['color'], $def['sort_order'], $active);
    $insStmt->execute();
    $insStmt->close();
...
$allowedStatuses = ['NEW', 'IN_PROGRESS', 'READY_TO_SHIP', 'READY_TO_INVOICE', 'INFO_REQUIRED', 'INFO_REQUESTED', 'COMMUNICATION', 'PENDING', 'HOLD', 'DELAY', 'PLASTICS_IN_STOCK'];
...
foreach ($policies as $p) {
    ...
    $stmt = $conn->prepare("
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    ");
    $description = 'Auto-generated v5 (' . date('Y-m-d') . ') - plny pokryt 15 kombinacii';
...
echo "\n--- Hotovo: {$created} novych policies, {$replaced} nahradenych ---\n";
