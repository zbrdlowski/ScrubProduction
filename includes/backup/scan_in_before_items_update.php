...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Decrease quantity from receiving shelf (A010)
$receivingShelf = 'A010';

if ($shelf_location !== $receivingShelf) {

    // Remove quantity from A010
    $stmt = $pdo->prepare("UPDATE stock_levels
        SET quantity = GREATEST(quantity - :qty, 0)
        WHERE item_id = :item_id
          AND shelf_name = :shelf_name
    ");
    $stmt->execute([
        'qty'        => $quantity,
        'item_id'    => $item['id'],
        'shelf_name' => $receivingShelf
    ]);

    // Also remove that amount from the total items table
    $stmt = $pdo->prepare("UPDATE items SET quantity = GREATEST(quantity - :qty, 0) WHERE id = :id");
    $stmt->execute([
        'qty' => $quantity,
        'id'  => $item['id']
    ]);
