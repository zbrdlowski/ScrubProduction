      ...
      <div class="info-box bg-purple">
        ...
        <div class="info-box-content">
          ...
          <h5><?php
            ...
            echo "$empty shelves  with zero quantity<br>$unassigned Part Numbers unassigned";
          ?></h5>
        </div>
      </div>
    </a>
    ...
    <a href="index.php?page=stock_movements">
      <div class="info-box bg-green">
        ...
        <div class="info-box-content">
          ...
          <h5><?php
            $stmt = $pdo->query("SELECT COUNT(*) AS recent FROM inventory_movements WHERE timestamp >= NOW() - INTERVAL 30 DAY");
...
$inTrend = $pdo->query("SELECT DATE(timestamp) AS day, SUM(quantity) AS qty
  FROM inventory_movements
  WHERE movement_type = 'IN' AND timestamp >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
...
$outTrend = $pdo->query("SELECT DATE(timestamp) AS day, SUM(quantity) AS qty
  FROM inventory_movements
  WHERE movement_type = 'OUT' AND timestamp >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
  GROUP BY DATE(timestamp)
  ORDER BY day ASC
")->fetchAll(PDO::FETCH_ASSOC);
?>
  ...
  <div class="col-md-6">
    <div class="info-box bg-green">
      ...
      <div class="info-box-content">
        ...
        <span class="info-box-number"><?= $inToday['total_qty'] ?> units (total scanned quantities)</span>
        <div class="progress">
          <div class="progress-bar" style="width:100%"></div>
        </div>
        <span class="progress-description">
          <?= $inToday['orders'] ?> orders / <?= $inToday['items'] ?> items (individual Part Numbers)
        </span>
      </div>
    </div>
    <canvas id="receivingTrend" height="120"></canvas>
  </div>
  ...
  <div class="col-md-6">
    <div class="info-box bg-red">
      ...
      <div class="info-box-content">
        ...
        <span class="info-box-number"><?= $outToday['total_qty'] ?> units (total scanned quantities)</span>
        <div class="progress">
          <div class="progress-bar" style="width:100%"></div>
        </div>
        <span class="progress-description">
          <?= $outToday['orders'] ?> orders / <?= $outToday['items'] ?> items (individual Part Numbers)
        </span>
      </div>
    </div>
    <canvas id="sendingTrend" height="120"></canvas>
  </div>
</div>
          ...
          <div class="col-md-6 col-sm-12">
            <div class="card">
              <div class="card-header bg-gradient-primary text-white">
                <h4><i class="fa fa-calendar"></i> Monthly Movement Trends</h4>
              </div>
              <div class="card-body">
          ...
          <div class="col-md-6 col-sm-12">
            <div class="card">
              <div class="card-header bg-gradient-info text-white">
                <h4><i class="fa fa-industry"></i> Supplier Barcode Distribution</h4>
              </div>
              <div class="card-body">
                <canvas id="supplierChart" height="180"></canvas>
              </div>
            </div>
          </div>
              ...
              <div class="panel panel-default">
                ...
                <div class="panel-body">
                  <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                      <?php
                      $stmt = $pdo->query("SELECT im.movement_type, im.order_id, im.item_name, im.shelf_name, im.quantity, im.operator, im.timestamp, i.name, i.description, i.color
          ...
          <?php foreach ($shelfGroup as $shelf): 
            ...
            ?>
                      <div class='col-xs-2 text-center' style='margin-bottom:10px;'>
                        <div style='background:<?= $color ?>; margin:1px; padding:10px; border-radius:4px; color:white;'>
